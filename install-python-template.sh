...
# Script para instalar el template de Python en HESTIA CP
...
# Función para crear el template de Python
create_python_template() {
    info "Creando template de Python..."
...
cat > $home_dir/conf/web/$domain/nginx.conf << 'NGINXEOF'
...
# Crear archivo de logrotate para la aplicación
cat > /etc/logrotate.d/python-app-$domain << 'LOGROTATEEOF'
$home_dir/web/$domain/logs/python/*.log {
    daily
    missingok
    rotate 14
    compress
...
info "Python application template successfully configured for $domain"
echo "================================================================"
...
echo "  Restart app: systemctl restart ${domain}_python.service"
echo "  Check status: systemctl status ${domain}_python.service"
echo "  View logs: journalctl -u ${domain}_python.service -f"
echo "  Install packages: $home_dir/web/$domain/.python-venv/bin/pip install <package>"
echo "================================================================"
EOF
    ...
    success "Template de Python creado en: $HESTIA_TEMPLATES_DIR/$TEMPLATE_NAME.tpl"
}
...
# Actualizar la configuración con el puerto correcto
...
verify_installation() {
    ...
    # Verificar template
    if [[ -f "$HESTIA_TEMPLATES_DIR/$TEMPLATE_NAME.tpl" ]]; then
        success "✓ Template encontrado: $TEMPLATE_NAME.tpl"
    else
    ...
    if [[ -x "$HESTIA_TEMPLATES_DIR/$TEMPLATE_NAME.tpl" ]]; then
        success "✓ Permisos correctos en el template"
    else
        error "✗ Permisos incorrectos en el template"
        ((errors++))
    fi
...
main() {
    echo
    echo "================================================================"
    echo "🧩 Hestia CP Python Template Installer"
    echo "================================================================"
    echo
    ...
    if verify_installation; then
        ...
        echo "  📁 Template: $HESTIA_TEMPLATES_DIR/$TEMPLATE_NAME.tpl"
        ...
        echo
        warning "Nota: El template estará disponible en Hestia CP como 'python-app'"
    else
